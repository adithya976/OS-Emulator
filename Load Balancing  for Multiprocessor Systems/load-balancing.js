...
addTooltip(algorithmSelect, 'Choose which load balancing algorithm to simulate');
addTooltip(processorsInput, 'Set number of processors (2-10) in your system');
addTooltip(weightsInput, 'Optional: Set relative weights for processors (comma-separated)');
addTooltip(taskListInput, 'Enter task durations (comma-separated numbers)');
addTooltip(randomTasksBtn, 'Generate random task durations');
addTooltip(runSimulationBtn, 'Run complete simulation automatically');
addTooltip(stepSimulationBtn, 'Advance simulation one step at a time');
addTooltip(resetSimulationBtn, 'Reset simulation to start over');
    ...
    function runSimulation() {
        ...
        switch (algorithm) {
            ...
            case 'weighted-rr':
                ...
                break;
            case 'least-response':
                simulationData = leastResponseTimeAlgorithm(tasks, processors, weights);
                break;
            case 'resource-based':
                simulationData = resourceBasedAlgorithm(tasks, processors, weights);
                break;
        }
    ...
    // Least Response Time Algorithm Implementation
    function leastResponseTimeAlgorithm(tasks, processors, weights) {
        const steps = [];
        const processorStates = Array(processors).fill().map((_, i) => ({
            weight: weights[i],
            tasks: [],
        ...
        while (true) {
            ...
            // Assign new tasks to processor with least response time
            if (time === 0) {
                // Initial assignment - assign to processor with least response time
                for (let i = 0; i < tasks.length; i++) {
                    // Find processor with least response time
                    let minResponse = Infinity;
                    let selectedProcessor = 0;
                    
                    for (let j = 0; j < processorStates.length; j++) {
                        if (processorStates[j].responseTime < minResponse) {
                            minResponse = processorStates[j].responseTime;
                            selectedProcessor = j;
                        }
                    }
                    ...
                    step.action = `Assigned task ${tasks[i]} to Processor ${selectedProcessor + 1} (least response time)`;
                }
            }
            ...
            for (let i = 0; i < processorStates.length; i++) {
                ...
                if (processor.currentTask && processor.currentTask.status === 'processing') {
                    ...
                    if (processor.currentTask.duration <= 0) {
                        ...
                        const taskResponseTime = processor.currentTask.endTime - processor.currentTask.assignedTime;
                        processor.responseTime = (processor.responseTime * (processor.tasks.length - 1) + taskResponseTime) / 
                                               processor.tasks.length;
                        
                        step.action += step.action ? '<br>' : '';
                        step.action += `Processor ${i + 1} completed task (response time: ${taskResponseTime})`;
                    }
                }
            }
    ...
    // Resource-Based Algorithm Implementation
    function resourceBasedAlgorithm(tasks, processors, weights) {
        const steps = [];
        const processorStates = Array(processors).fill().map((_, i) => ({
            weight: weights[i],
            tasks: [],
        ...
        while (true) {
            ...
            // Assign new tasks based on current load
            if (time === 0) {
                // Initial assignment - assign to least loaded processor
                for (let i = 0; i < tasks.length; i++) {
                    ...
                    for (let j = 0; j < processorStates.length; j++) {
                        // Calculate load as (total time + pending tasks) / weight
                        const pendingTasks = processorStates[j].tasks.filter(t => 
                            t.status !== 'completed').length;
                        const load = (processorStates[j].totalTime + pendingTasks) / weights[j];
            ...
            for (let i = 0; i < processorStates.length; i++) {
                ...
                // Update current load
                const pendingTasks = processor.tasks.filter(t => t.status !== 'completed').length;
                processor.load = (processor.totalTime + pendingTasks) / weights[i];
            }
